 ...
 */
package cn.zhouzy.greatcate.common.view.pullview;
...
public class CommPullToRefreshView extends LinearLayout
{
	...
	/** header view. */
	private CommListViewHeader mHeaderView;

	/** footer view. */
	private CommListViewFooter mFooterView;
	 ...
	 *            the attrs
	 */
	public CommPullToRefreshView(Context context, AttributeSet attrs)
	{
		super(context, attrs);
		init(context);
	}
	 ...
	 *            the context
	 */
	public CommPullToRefreshView(Context context)
	{
		super(context);
		init(context);
	}
	...
	{
		mHeaderView = new CommListViewHeader(mContext);
	...
	private void addFooterView()
	{
		...
		mFooterView = new CommListViewFooter(mContext);
	...
	private boolean isRefreshViewScroll(int deltaY)
	{
		...
		if (mAdapterView != null)
		{
			...
			} else if (deltaY < 0)
			{
				...
				if (lastChild.getBottom() <= getHeight() && mAdapterView
						.getLastVisiblePosition() == mAdapterView.getCount() - 1)
				{
					mPullState = PULL_UP_STATE;
					return true;
				}
	...
	private void headerPrepareToRefresh(int deltaY)
	{
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != CommListViewHeader.STATE_REFRESHING)
		{
			// 提示松开刷新
			mHeaderView.setState(CommListViewHeader.STATE_READY);

		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight)
		{
			// 提示下拉刷新
			mHeaderView.setState(CommListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY)
	{
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView
				.getState() != CommListViewFooter.STATE_LOADING)
		{
			mFooterView.setState(CommListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight))
		{
			mFooterView.setState(CommListViewFooter.STATE_LOADING);
		}
	}
	...
	private int updateHeaderViewTopMargin(int deltaY)
	{
		...
		if (deltaY > 0 && mPullState == PULL_UP_STATE && Math.abs(
				params.topMargin) <= mHeaderViewHeight)
		{
			return params.topMargin;
		}
		// 同样地,对下拉做一下限制,避免出现跟上拉操作时一样的bug
		if (deltaY < 0 && mPullState == PULL_DOWN_STATE && Math.abs(
				params.topMargin) >= mHeaderViewHeight)
		{
			return params.topMargin;
		}
	...
	public void headerRefreshing()
	{
		...
		mHeaderView.setState(CommListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish()
	{
		...
		if (mAdapterView != null)
		{
			...
			{
				mFooterView.setState(CommListViewFooter.STATE_EMPTY);
			}
		} else
		{
			mFooterView.setState(CommListViewFooter.STATE_READY);
	...
	public void onFooterLoadFinish()
	{
		...
		if (mAdapterView != null)
		{
			...
			{
				mFooterView.setState(CommListViewFooter.STATE_NO);
			}
		} else
		{
			mFooterView.setState(CommListViewFooter.STATE_READY);
	 ...
	 */
	public CommListViewHeader getHeaderView()
	{
		return mHeaderView;
	}
	 ...
	 */
	public CommListViewFooter getFooterView()
	{
		return mFooterView;
	}
	...
	public boolean getHeaderRefreshStatus()
	{
