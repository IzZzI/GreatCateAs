...
public class ImageShowPickerAdapter extends RecyclerView.Adapter<ImageShowPickerAdapter.ViewHolder> implements ImageShowPickerPicListener
{
	...
	public void onBindViewHolder(ViewHolder holder, int position)
	{
		...
		} else
		{
			...
			} else if (null != list.get(position).getImageShowPickPath() && !"".equals(list.get(position).getImageShowPickPath()))
			{
				Drawable drawable = Drawable.createFromPath(list.get(position).getImageShowPickPath());
				((ImageView) holder.iv_pic).setImageDrawable(drawable);
			} else
			{
